...
- **The overlay is the exception to that line, and it is not a reversal.**
  ...
  resident sysmodule in a `0x100000` heap, so the footprint M0-1 was measuring
...
1. **The bsd transfer memory**, allocated from *our* heap by `socketInitialize`,
   ...
   `sys-rommsync`'s inner heap is `0x100000` (1 MiB), derived term by term in
...
and `state::kMaxRecords`, which are sized against the ~908 KiB this section
...
directions. The inner heap is `0x100000` with ~908 KiB left after the trimmed bsd
