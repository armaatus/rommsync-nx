...
int Stalled(http::HttpClient& client, const std::string& base, const Fixture& fixture) {
  ...
  {
    // Two stalls, so the second retry is what proves the doubling. `claim`
    ...
    // there was one is a test nobody runs, and a sysmodule parks on its own
    // primitive anyway.
    ...
    checks.Expect(negotiated.ok(),
                  "the third attempt reaches the real server: " + negotiated.message);
    checks.ExpectEq(negotiated.attempts, 3, "two stalls cost two retries");
    checks.ExpectEq(waits.size(), std::size_t{2}, "and each retry waited first");
    if (waits.size() == 2) {
      checks.ExpectEq(waits[0].count(), std::int64_t{50}, "the first backoff");
      checks.ExpectEq(waits[1].count(), std::int64_t{100}, "...doubled for the second");
    }
    checks.ExpectEq(negotiated.waited.count(), std::int64_t{150},
  ...
  // pairing screen.
  {
    ...
    checks.ExpectEq(lost.attempts, 1, "with no wait injected there is no retry");
