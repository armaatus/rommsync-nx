...
phase_layout() {
  ...
  # The layout, path by path. Sorted on both sides so this is about the set and
  # not about the order -- `deterministic` is what holds the order.
  local expected
  expected="$(printf '%s\n' \
    "LICENSE" \
    "README.txt" \
    ...
    "switch/.overlays/ovl-rommsync.ovl" | sort)"
  [ "$(sort <<<"$entries")" = "$expected" ] || {
    echo "--- in the archive ---"; sort <<<"$entries"
...
phase_refuses() {
  ...
  [ ! -e "$zip" ] || fail "a refused build still left $zip behind"
  ...
  while IFS= read -r line; do
    ...
    case "$path" in
      *"/atmosphere/contents/$tid/exefs.nsp"|*"/switch/.overlays/ovl-rommsync.ovl")
        continue ;;  # replaced on purpose; there were none of these to begin with
    esac
