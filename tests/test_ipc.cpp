...
// `FakeEngine` is `tests/fake_engine.hpp`'s: `overlay.*` drives the same
// service through the overlay's real IPC client, and two copies of it would be
// two things to keep in step (M9-7, #198).
using fakes::FakeEngine;
...
/// of their `config.ini`. The heap is `0x150000` with a download buffer and the
